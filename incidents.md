...
Each incident is a level-2 heading whose status word is `open` until the episode
recovers, and `recovered` afterwards:
...
The template above is indented rather than written as a real heading, so that
`grep -c "^## Incident" incidents.md` counts actual incidents only.
